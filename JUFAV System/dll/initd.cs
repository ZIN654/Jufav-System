...
namespace JUFAV_System.dll
{
       ...
       
    {
        ...
        //rename some tables
        /*
        public void ArcInitializeTable()
        {
            //try reverse where the users has the foreign
            String[] Querys = { "DROP TABLE SAMPLE;", "CREATE TABLE USERS(USERID INT NOT NULL PRIMARY KEY);", "CREATE TABLE USER_INFO(USERINFOID INT NOT NULL PRIMARY KEY,USERIDS INT,NAME VARCHAR(50),USERNAME VARCHAR(50),EMAIL VARCHAR(50),PASSWORDS VARCHAR(50),ROLES VARCHAR(2),FOREIGN KEY(USERIDS)REFERENCES USERS(USERID) ON DELETE CASCADE);", "CREATE TABLE MAINMODULES(MODULEID INT NOT NULL PRIMARY KEY,USERID INT,MODULENAME VARCHAR(50), FOREIGN KEY (USERID) REFERENCES USERS (USERID) ON DELETE CASCADE);", "CREATE TABLE SUBMODULES(SUBMODULEID INT NOT NULL PRIMARY KEY,USERID INT,SUBMODULENAME VARCHAR(50),HASACCESS VARCHAR(2), FOREIGN KEY (USERID) REFERENCES USERS (USERID) ON DELETE CASCADE);" };
            scon.ConnectionString = @"Data Source=" + Constring;
        ...
        public void ArcInitTableFilemaintenance()
        {
            //USE MARKUP ID TO IDENTIFY PARA SA PRODUCTS
            //if di nya inispescify ung value ng markup sa text boxthen query nalng ung id ng subcategory
            //chech mo muna if may laman ung supplier ay may laman.
        ...
        */
        public void ArcInitTableInventory()
        {
            ...
            String[] Querys = { "CREATE TABLE ARCSTOCKADJUSTMENTS(STOCKADJUSTMENTID INT NOT  NULL PRIMARY KEY,USERID INT,DATEOFADJUSTMENT VARCHAR(12),PRODUCTID INT,PRODUCTNAME VARCHAR(50),ADJUSTMENTTYPE VARCHAR(2),PREVIOUSQUANTITY FLOAT,ADJUSTEDQUANTITY FLOAT,REASON VARCHAR(50),OTHERS VARCHAR(100),FOREIGN KEY (USERID) REFERENCES USERS(USERID),FOREIGN KEY (PRODUCTID) REFERENCES PRODUCTS(PRODUCTID))", "CREATE TABLE ARCPURCHASEORDER(POID INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT,USERID INT,ORDERDATE VARCHAR(50),EXPECTEDORDERDATE VARCHAR(50),SUPPLIER VARCHAR(50),TIMES VARCHAR(50),TOTALPRODUCTS VARCHAR(50),TOTALCOST FLOAT,ORDERSTATUS VARCHAR(2),FOREIGN  KEY (USERID) REFERENCES USERS(USERID) ON DELETE CASCADE);", "CREATE TABLE ARCPOITEMORDERTABLE(ORDERID INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT,USERID iNT,POID INT,ITEMID INT,QUANTITY INT,PRODUCTNAME VARCHAR(50),ORIGINALPRICE FLOAT,TOTAL FLOAT,FOREIGN KEY (USERID) REFERENCES USERS(USERID)ON DELETE CASCADE,FOREIGN KEY (POID) REFERENCES ARCPURCHASEORDER(POID) ON DELETE CASCADE,FOREIGN KEY (ITEMID) REFERENCES PRODUCTS(PRODUCTID)ON UPDATE CASCADE);", "CREATE TABLE ARCPOVENDORINFO(ID INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT,POID INT,COMPANYNAME VARCHAR(50),CONTACTPERSON VARCHAR(50),CONTACTNO VARCHAR(50),COMPANYADDRESS VARCHAR(50),FOREIGN KEY(POID) REFERENCES ARCPURCHASEORDER(POID) ON DELETE CASCADE);", "CREATE TABLE ARCPOSHIPTOINFO(ID INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT,POID INT,COMPANYNAME VARCHAR(50),CONTACTPERSON VARCHAR(50),CONTACTNO VARCHAR(50),COMPANYADDRESS VARCHAR(50),REMARKS VARCHAR(100),FOREIGN KEY(POID) REFERENCES ARCPURCHASEORDER(POID) ON DELETE CASCADE);" };
            scon.ConnectionString = @"Data Source=" + Constring;
            scon.Open();
            foreach (String i in Querys)
            {
